...
function TweetActionLike({LikeCount,onLikeTweet, id}) {

  const {tweets} = useContext(TweetContext);
  const {user} = useContext(UserContext);

  const userLiked = tweets.likes && tweets.likes.includes(user.id);
  


 return (

    <button className={ `tweet-action ${userLiked ? 'liked' : ''    }   `  } onClick={() => onLikeTweet(id)} >
      <svg width="24" height="24" viewBox="0 0 24 24" fill="" xmlns="http://www.w3.org/2000/svg" className="actionLike"  >
